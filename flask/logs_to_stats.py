...
def auth_log_to_df(path_to_auth_log, path_to_country_code_table):
    ...
    country_code_df = pandas.read_csv(
        path_to_country_code_table, header=None, names=["country code", "name"]
    )
        ...
        if ip:
            ...
            if match:
                return country_code_df[
                    country_code_df["country code"] == match.country
                ]["name"].values[0]
            else:
                return None
        else:
    ...
    for indx, line in enumerate(lines):
        ...
        if "[" in line_as_list[1]:  # https://serverfault.com/a/526151
            ...
            line_as_dict["pid"] = (
                line_as_list[1].split("[")[1].replace("]:", "").strip()
            )
        else:
    ...
    df["datetime"] = pandas.to_datetime(
        "2020 " + df["date time"], format="%Y %b %d %H:%M:%S"
    )
...
def plot_username_distribution(df, path_to_save_to: str, output_filename: str) -> None:
    ...
    _ = plt.xticks(
        range(len(user_name_series)), list(user_name_series.index), rotation="vertical"
    )
...
def plot_ip_vs_time(df, path_to_save_to: str, output_filename: str) -> None:
    ...
    gb = (
        df.groupby([(df["year"]), (df["month"]), (df["day"])])
        .nunique()["ip"]
        .plot(kind="bar")
    )
...
def plot_country_per_day_vs_time(
    df, path_to_save_to: str, output_filename: str
) -> None:
    """
    ...
    gb = df.groupby([(df["year"]), (df["month"]), (df["day"]), (df["country"])])[
        "ip"
    ].nunique()
    gb_reduced = gb[gb > 4]
